...
func buildTestData(args []string, silent bool, printUsage func(io.Writer)) map[string]TestData_Package {
	...
	for i := 0; i < numPkgs; i++ {
		...
		details := buildPackageDetailsData(&analyzer, pkg.Path())
...
func GenTestData(outputDir string, args []string, goldVersion string, silent bool, printUsage func(io.Writer)) {
