...
type SourceFileInfo struct {
	...
	// The following two are blank for most files.
...
func (d *CodeAnalyzer) BuildCgoFileMappings(pkg *Package) {
	...
	for i, compiledFile := range pkg.PPkg.CompiledGoFiles {
		...
		if info == nil {
			log.Println(compiledFile, "has no original file.")
			continue
		}

		if info.OriginalGoFile != "" && info.GeneratedFile != info.OriginalGoFile {
			d.generatedFile2OriginalFileTable[info.GeneratedFile] = info.OriginalGoFile
		}
